...
import { CreateSpacePayload } from '@/lib/validators/space'
import { useMutation } from '@tanstack/react-query'
import axios, { AxiosError } from 'axios'
import { useRouter } from 'next/navigation'
import { useState } from 'react'
import { validateSpaceName } from '@/lib/validators/inputValidators'
...
const Page = () => {
  ...
  const { mutate: createCommunity, isLoading } = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await axios.post('/api/space', payload)
    ...
    <div className='container flex items-center h-full max-w-3xl mx-auto'>
      <div className='relative bg-white w-full h-fit p-4 rounded-lg space-y-6'>
        ...
        <div className='flex justify-end gap-4'>
          ...
          <Button
            isLoading={isLoading}
            disabled={input.length === 0 || validateSpaceName(input) === false}
